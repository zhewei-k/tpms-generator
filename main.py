...
def meshConvergence(lat, cell, thick, final_elementmultiplier:float, initial_elementmultiplier:float, step:float, force:float):
    count = 0
    ...
    final_elementmultiplier = int(final_elementmultiplier*1000)
    initial_elementmultiplier = int(initial_elementmultiplier*1000)
    step = int(step*1000)
    multiplier_list = range(final_elementmultiplier, initial_elementmultiplier, -step)
    ...
    return multiplier_list, stiffness_list
...
def main():
    ...
    thickness_final, stiffness_final = secantOptimising(0.12, 10, 0.02, 100)
    # thickness_final, stiffness_final = listProcessing(0.25,0.15,0.1,100)

    print("---- TPMS ANALYSIS COMPLETE ----")
    print("Data has been stored in ./data/")
    print(f"thickness: {thickness_final} mm")
    print(f"stiffness: {stiffness_final / 1e9} GPa")
